...
export default function ClinicErrorsTable({ title, subtitle, apiUrl, columns: initialColumns, editable = false, onColumnsChange }: Props) {
  ...
  const commitColumn = async (idx: number, val: string) => {
    ...
    const updatedCols = columns.map((c, i) => i === idx ? { ...c, key, label } : c);
  ...
  const setColumnType = async (idx: number, type: ClinicErrorType | "") => {
          ...
          <thead className="sticky top-0 z-20">
            <tr className="border-b border-white/8" style={{ background: "var(--sticky-cell-bg)" }}>
              ...
              {columns.map((col, ci) => (
                <th
                  key={col.key}
                  className="px-3 py-3 text-white/50 font-medium text-xs text-center leading-tight align-top"
                  style={{ minWidth: 110, maxWidth: 140, background: "var(--sticky-cell-bg)" }}
                >
                  <div className="flex flex-col gap-1.5 items-center">
                    ...
                    {editable ? (
                      <select
                        value={col.type || ""}
                        onChange={e => setColumnType(ci, e.target.value as ClinicErrorType | "")}
                        className={`text-[10px] font-medium rounded-md px-1.5 py-0.5 border outline-none cursor-pointer transition-colors
                          ${col.type ? TYPE_STYLES[col.type] : "bg-white/5 text-white/40 border-white/10 hover:bg-white/10"}`}
                      >
                        <option value="" className="bg-gray-900 text-white/60">Тип…</option>
                        {TYPE_OPTIONS.map(t => (
                          <option key={t} value={t} className="bg-gray-900 text-white">{t}</option>
                        ))}
                      </select>
                    ) : col.type ? (
                      <span className={`text-[10px] font-medium rounded-md px-1.5 py-0.5 border ${TYPE_STYLES[col.type]}`}>
                        {col.type}
                      </span>
                    ) : null}
                  </div>
